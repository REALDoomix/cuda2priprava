...
__global__ void kernel_scale(CudaImg src, CudaImg dst, float scale_x, float scale_y) {
    ...
    uchar4 rgba00 = src.at4(int_orig_y, int_orig_x);
    uchar4 rgba01 = src.at4(int_orig_y, int_orig_x + 1);
    uchar4 rgba10 = src.at4(int_orig_y + 1, int_orig_x);
    uchar4 rgba11 = src.at4(int_orig_y + 1, int_orig_x + 1);

    uchar4 rgba;
    rgba.x = rgba00.x * (1 - diff_y) * (1 - diff_x) +
             rgba01.x * (1 - diff_y) * diff_x +
             rgba10.x * diff_y * (1 - diff_x) +
             rgba11.x * diff_y * diff_x;

    rgba.y = rgba00.y * (1 - diff_y) * (1 - diff_x) +
             rgba01.y * (1 - diff_y) * diff_x +
             rgba10.y * diff_y * (1 - diff_x) +
             rgba11.y * diff_y * diff_x;

    rgba.z = rgba00.z * (1 - diff_y) * (1 - diff_x) +
             rgba01.z * (1 - diff_y) * diff_x +
             rgba10.z * diff_y * (1 - diff_x) +
             rgba11.z * diff_y * diff_x;

    rgba.w = rgba00.w * (1 - diff_y) * (1 - diff_x) +
             rgba01.w * (1 - diff_y) * diff_x +
             rgba10.w * diff_y * (1 - diff_x) +
             rgba11.w * diff_y * diff_x;
